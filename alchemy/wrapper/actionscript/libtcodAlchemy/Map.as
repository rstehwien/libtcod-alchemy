package libtcodAlchemy 
{
	...
	*/
	public class Map
	{
		...
		private var _alchemyMap:uint;
		private var _width:uint;
		private var _height:uint;

		public function Map(width:uint = 0, height:uint = 0, defaultCell:MapCell = null)
		{
			setSize(width, height, defaultCell, false);
		}
		...
		public function copy(map:Map):void
		{
			...
			{
				setSize(_width, _height, null, false);
			}
		}
		...
		public function setSize(newWidth:uint, newHeight:uint, defaultCell:MapCell = null, copyOld:Boolean = true):void
		{
			if (defaultCell == null)
			{
				defaultCell = new MapCell();
			}
			var defaultByte:uint = defaultCell.byte;
		...
		private function newAlchemyMap():void
		{
			disposeAlchemyMap();
			_alchemyMap = libtcod_wrapper.map_new(width, height);
			libtcod_wrapper.map_read_bytearray(_alchemyMap, _data);
		}

		private function disposeAlchemyMap():void
		{
			if (_alchemyMap != 0)
			{
				libtcod_wrapper.map_delete(_alchemyMap);
				_alchemyMap = 0;
			}
		}

		public function get alchemyMap():uint { return _alchemyMap; }

		public function computeFov(x:int, y:int, maxRadius:int, lightWalls:Boolean, algorithm:int):void
		{
			libtcod_wrapper.map_read_bytearray(_alchemyMap, _data);
			libtcod_wrapper.map_compute_fov(_alchemyMap, x, y, maxRadius, lightWalls, algorithm);
			libtcod_wrapper.map_write_bytearray(_alchemyMap, _data);
		}
		...
		public function clearVisible():void
		{
			...
			libtcod_wrapper.map_read_bytearray(_alchemyMap, _data);
			libtcod_wrapper.map_clear_visible(_alchemyMap);
			libtcod_wrapper.map_write_bytearray(_alchemyMap, _data);
		}
		...
		public function clearSeen():void
		{
			// Faster to call the C code for large data-sets
			libtcod_wrapper.map_read_bytearray(_alchemyMap, _data);
			libtcod_wrapper.map_clear_seen(_alchemyMap);
			libtcod_wrapper.map_write_bytearray(_alchemyMap, _data);
		}
